...
const isPrUrl = computed(() => {
  return PR_URL_REGEX.test(rawDiff.value.trim())
})
...
async function handleSubmit() {
  ...
  try {
    if (isPrUrl.value) {
      await review.submitByUrl({ url: rawDiff.value.trim() })
    } else {
      await review.submitRawDiff({
        rawDiff: rawDiff.value,
        ...meta.value,
      })
    }
  } catch (e) {
