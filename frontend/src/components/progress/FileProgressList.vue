...
watch(() => review.filesInProgress.value.length, async () => {
  await nextTick()
  if (listEl.value) {
...
<template>
  <div v-if="review.filesInProgress.value.length > 0" class="mt-10">
    <h3 class="text-sm font-semibold text-slate-700 mb-3">Files</h3>
    <div ref="listEl" class="max-h-64 overflow-y-auto space-y-1 rounded-lg border border-slate-200 bg-white p-2">
      <div v-for="file in review.filesInProgress.value" :key="file.path"
