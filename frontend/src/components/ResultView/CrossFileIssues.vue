...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
    <h3 class="text-sm font-semibold text-slate-900 mb-3">跨文件问题</h3>
    ...
    <div v-else class="space-y-2">
      <div v-for="(issue, idx) in issues" :key="idx"
        ...
        <button type="button" @click="openIdx = openIdx === idx ? null : idx"
          ...
          <svg class="w-4 h-4 text-slate-400 transition-transform shrink-0" :class="openIdx === idx ? 'rotate-180' : ''"
